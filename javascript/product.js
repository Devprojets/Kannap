...
function handleAddTocart(){
  ...
  if (oldCartItems.some((item)=>item.id===product_id)) {
    ...
    //recupe l'index du produit déja ajouté au panier selectionner dan sle tableau 
    ...
    if(index == -1){
     ...
     alert("Le produit a été ajoute au panier ")
       ...
       //le produit existe déja donc augmenter la quantité//
...
// si le produit n'existe pas dans le panier on le rajoute//
  } else {
    oldCartItems.push({
   /*  "newIdSet" : product_id +'itemColor'+color_selected, */
      "color": color_selected,
